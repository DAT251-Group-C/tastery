...
export class Migration1712597716018 implements MigrationInterface {
  name = 'Migration1712597716018';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "recipes" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "userId" uuid NOT NULL, "name" character varying(255) NOT NULL DEFAULT '', "description" character varying(255) NOT NULL DEFAULT '', "instructions" character varying(2083) NOT NULL DEFAULT '', "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "tags" text NOT NULL DEFAULT '[]', "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, CONSTRAINT "PK_8f09680a51bf3669c1598a21682" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."ingredients_unit_enum" AS ENUM('g', 'kg', 'ml', 'l', 'unit', 'tsp', 'tbsp', 'clove', 'pinch', 'slice', 'cup')`,
    );
    await queryRunner.query(
      `CREATE TABLE "ingredients" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "ean" character varying(63), "name" character varying(255) NOT NULL DEFAULT '', "image" character varying(2083), "recipeId" uuid NOT NULL, "amount" numeric NOT NULL DEFAULT '0', "unit" "public"."ingredients_unit_enum" NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, CONSTRAINT "PK_b55414f4f386f632ce202709cf5" PRIMARY KEY ("id", "recipeId"))`,
    );
    await queryRunner.query(
