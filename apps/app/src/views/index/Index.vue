      ...
      <div class="w-full">
        ...
        <div :class="['grid grid-cols-2 my-4 gap-4']">
          <RouterLink
            ...
            :key="recipe.id"
            :to="`/recipe/${recipe.id}`"
            class="flex flex-col ring-1 ring-neutral-300 p-4 gap-y-2 rounded-sm h-[136px] hover:bg-neutral-200 hover:ring-primary"
          >
...
const search = ref('');
const { data, hasNextPage, isLoading } = useRecipes(search);

const filterTags = ['Italian', 'Pasta', 'Pizza', 'Dessert', 'Healty'];
